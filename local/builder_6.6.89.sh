...
if [[ "$APPLY_BETTERNET" == "y" || "$APPLY_BETTERNET" == "Y" ]]; then
  ...
  # eBPF 增强配置
  echo "CONFIG_CGROUP_BPF=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_EVENTS=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_KPROBE_OVERRIDE=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_LSM=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_DEBUG_INFO_BTF=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_DEBUG_INFO_BTF_MODULES=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_PRELOAD=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_PRELOAD_UMD=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_STACKMAP=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_ARRAY=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_HASH=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_LRU=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_PERCPU=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_RINGBUF=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_BLOOM_FILTER=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_NET_SCH_INGRESS=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_NET_CLS_BPF=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_NET_ACT_BPF=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPFILTER=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPFILTER_UMH=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_NET_SOCK_MSG=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_XDP_SOCKETS=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_XDP_SOCKETS_DIAG=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_PERF_EVENTS=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_HAVE_PERF_EVENTS=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_HAVE_SAMPLE_FTRACE_DIRECT=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_HAVE_SAMPLE_FTRACE_DIRECT_MULTI=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_KUNIT_TEST=m" >> "$DEFCONFIG_FILE"
  echo "CONFIG_BPF_LIRC_MODE2=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_KPROBES=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_KRETPROBES=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_KPROBE_EVENTS=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_UPROBES=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_UPROBE_EVENTS=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_DYNAMIC_EVENTS=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_FTRACE=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_FUNCTION_TRACER=y" >> "$DEFCONFIG_FILE"
  echo "CONFIG_FUNCTION_GRAPH_TRACER=y" >> "$DEFCONFIG_FILE"
